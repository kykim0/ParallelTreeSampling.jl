using Distributions
using ImportanceWeightedRiskMetrics
using ParallelTreeSampling
using POMDPModelTools
using POMDPPolicies
using POMDPs
using POMDPSimulators
using Random


# Runs Monte Carlo baseline.
function run_baseline(amdp, s, distrib_fn; N=1000)
    function_policy = (s) -> rand(distrib_fn(amdp, s))
    costs = [sum(collect(simulate(HistoryRecorder(), amdp, function_policy, s)[:r]))
             for _ in 1:N]
    output = (costs, [])
    return output
end


# Runs MCTS-based sampling.
function run_mcts(amdp, s, distrib_fn; N=1000, c=0.3, vloss=0.0, α=0.1, β=1.0, γ=0.0)
    solver = PISSolver(; depth=100,
                       exploration_constant=c,
                       n_iterations=N,
                       enable_action_pw=false,  # Needed for discrete cases.
                       k_state=Inf,             # Needed for discrete cases (to always transition).
                       virtual_loss=vloss,
                       nominal_distrib_fn=distrib_fn,
                       show_progress=true,
                       tree_in_info=true,
                       α=α);
    planner = solve(solver, amdp);
    a, info = action_info(planner, s; tree_in_info=true, β=β, γ=γ)
    tree = info[:tree]
    output = (tree.costs, [], tree.weights, info)
    return output, planner
end


# Returns risk metrics.
function eval_metrics(costs; weights=nothing, alpha=1e-4)
    costs = [Float64(cost) for cost in costs]
    if weights == nothing
        weights = ones(length(costs))
    else
        weights = [Float64(weight) for weight in weights]
    end

    metrics = IWRiskMetrics(costs, weights, alpha);
    return metrics
end
